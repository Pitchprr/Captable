                                                                ...
                                                                <div className="space-y-1">
                                                                    {payload.map((entry, index) => (
                                                                        <div key={index} className="flex items-center justify-between gap-8">
                                                                            ...
                                                                            <span className="text-xs font-mono font-bold text-slate-700">{formatCurrency(Number(entry.value))}</span>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                    ...
                                    <table className="w-full text-sm text-left border-collapse">
                                        <thead className="text-[10px] text-slate-500 uppercase tracking-wider bg-slate-50/80 border-b border-slate-200">
                                            <tr>
                                                ...
                                                {isCarveOutActive && carveOutPercent > 0 && <th className="px-4 py-3 font-bold text-right text-amber-600">Carve-Out</th>}
                                                    ...
                                                    return (
                                                        <>
                                                            ...
                                                            >
                                                                ...
                                                                {isCarveOutActive && carveOutPercent > 0 && <td className="px-4 py-2 text-right font-mono font-bold text-xs text-amber-600">{formatCurrency(groupTotalCarve)}</td>}
                                                            ...
                                                            {isExpanded && groupPayouts.sort((a, b) => b.totalPayout - a.totalPayout).map((p) => (
                                                                <tr key={p.shareholderId} className="hover:bg-slate-50 transition-colors group">
                                                                    ...
                                                                    {isCarveOutActive && carveOutPercent > 0 && (
                                                ...
                                                return (
                                                    <>
                                                        ...
                                                        <tr className="bg-slate-900 text-white font-bold">
                                                            ...
                                                            {isCarveOutActive && carveOutPercent > 0 && <td className="px-4 py-4 text-right font-mono text-xs text-amber-400">{formatCurrency(payouts.reduce((sum, p) => sum + p.carveOutPayout, 0))}</td>}
                                                            ...
                                                            {step.details?.shareholders && step.details.shareholders.length > 0 && (
                                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                                    <div>
                                                                        ...
                                                                        {/* Total for this step */}
                                                                        <div className="mt-2 pt-3 border-t border-slate-200 flex justify-between items-center text-xs font-bold text-slate-900 px-2 bg-slate-100/30 rounded-b">
                                                                            <span className="uppercase tracking-wider">Step Total</span>
                                                                            <span className="font-mono text-blue-600">{formatCurrency(step.amount)}</span>
                                                                        </div>
                                                                    </div>

                                                                    {/* Summary by Category */}
                                                                    <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                                                                        <h5 className="text-[10px] uppercase font-bold text-slate-400 mb-4 flex items-center gap-1">
                                                                            <Sigma className="w-3 h-3" /> Summary by Group
                                                                        </h5>
                                                                        <div className="space-y-3">
                                                                            {(() => {
                                                                                const categories = [
                                                                                    { name: 'Founders', color: 'bg-blue-500', group: 'Founder' },
                                                                                    { name: 'Investors', color: 'bg-emerald-500', group: 'Investor' },
                                                                                    { name: 'Team', color: 'bg-amber-500', group: 'Team' }
                                                                                ];
                                                                                    ...
                                                                                    const groupAmount = step.details!.shareholders.reduce((sum, s) => {
                                                                                        ...
                                                                                        const isMatch = cat.group === 'Founder' ? role === 'Founder' :
                                                                                            cat.group === 'Team' ? ['Employee', 'Advisor'].includes(role || '') :
                                                                                                (role !== 'Founder' && !['Employee', 'Advisor'].includes(role || ''));
                                                                                        return isMatch ? sum + s.amount : sum;
                                                                                    }, 0);

                                                                                    const percent = step.amount > 0 ? (groupAmount / step.amount) * 100 : 0;
                                                                                    ...
                                                                                    return (
                                                                                        <div key={cat.name} className="space-y-1.5">
                                                                                            <div className="flex justify-between text-[11px] font-bold">
                                                                                                ...
                                                                                                <span className="text-slate-900">{formatCurrency(groupAmount)}</span>
                                                                                            </div>
                                                                                            ...
                                                                                            <div className="text-[10px] text-right text-slate-400">{percent.toFixed(1)}% of step</div>
                                                                                        </div>
                                                                                    );
                                                                                });
                                                                            })()}
